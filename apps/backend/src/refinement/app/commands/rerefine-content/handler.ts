...
import { RerefineContentCommand } from '@refinement/app/commands/rerefine-content/command';
import { RerefineContentResult } from '@refinement/app/commands/rerefine-content/result';
 ...
 *    - Add to aggregate
 * 7. Mark refinement as completed
 ...
 */
@Injectable()
@CommandHandler(RerefineContentCommand)
export class RerefineContentCommandHandler implements ICommandHandler<
...
> {
  ...
  /**
   * Executes the re-refine content command
   *
   * @param command - The command to execute
   * @returns Result with re-refinement status and metrics
   */
  async execute(
    command: RerefineContentCommand,
  ): Promise<RerefineContentResult> {
    const { contentItemId, reason, config } = command;

    this.logger.log(
      `Starting re-refinement for content item: ${contentItemId}, reason: ${reason}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Re-refinement failed for ${contentItemId}: ${error instanceof Error ? error.message : String(error)}`,
  ...
  private async processChunks(
    chunks: Chunk[],
    contentItem: ContentItemData,
    config: RerefineContentCommand['config'],
  ): Promise<Chunk[]> {
